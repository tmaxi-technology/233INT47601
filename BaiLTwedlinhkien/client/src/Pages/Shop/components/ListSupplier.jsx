...
import { dataListSupplier } from "../../../Share/dataFake"
import { useState } from "react"
const ListSupplier = ({ onOpen, onClose }) => {
    ...
    return (
        <div>
            <div>
                <button onClick={onClose} className="flex justify-between w-full gap-x-1.5 rounded-md bg-white px-3 py-3 text-base font-bold text-gray-900 shadow-sm lg:border-2 border-b-2 ring-gray-300 hover:bg-gray-50">
                    Nhà cung cấp
            ...
            <div className={` ${!onOpen ? "block h-full" : "hidden h-0"} transition-all ease-linear duration-300 `}>
                <div className="w-full z-10 origin-top-right lg:rounded-b-md rounded-none bg-white lg:shadow-lg shadow-none lg:border border-none focus:outline-none">
                    <div className="py-1">
                        <ul>
                            {
                                dataListSupplier && dataListSupplier.map((val, key) => (
